    ...
    <div class="invoice overflow-auto">
        <div style="min-width: 600px">
            ...
            <main>
                ...
                <div>
                    <div @click="insertNewProductModal = true" class="btn btn-primary">Add new item to invoice</div>
                </div>
        ...
        <template slot="body">
            <form @submit="addNewRowToInvoice">
                <div class="form-group">
                    ...
                    <input v-model="newItem.quantity" id="itemQuantity" type="number" class="form-control" required>
                </div>
                <div class="form-group">
                    ...
                    <select v-model="newItem.invoice_item_product_id" id="productItem" class="form-control" required>
                        <option v-for="invoiceItemProduct in invoiceItemProducts" :key="invoiceItemProduct.id" :value="invoiceItemProduct.id">{{invoiceItemProduct.name}}</option>
                    </select>
                </div>
                <button type="submit" class="btn btn-primary">Add button to invoice</button>
            </form>
        </template>
...
export default {
    ...
    methods: {
        ...
        getAllInvoiceItemProducts() {
            const self = this;
            Vue.axios.get('/invoice/invoiceitemproducts/get').then(response => {
                    var data = response.data;
                    if (data.success == true) {
